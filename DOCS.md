...
- **Только точное совпадение для PDF:** Логика проверки на дубликаты при загрузке PDF-файлов была изменена. Вместо нечеткого поиска (`similarity`) теперь используется только проверка на **точное, 100% совпадение** текста вопроса. Это сделано на основании того, что вопросы от преподавателей являются выверенным материалом, и даже небольшие отличия в формулировках могут быть важны. Это изменение предотвращает ошибочную фильтрацию уникальных вопросов.
  - *Исправление (10.07.2025):* Добавлен пропущенный метод `question_exists_exact` в слой `DatabaseFacade` для корректной работы этой функции.
