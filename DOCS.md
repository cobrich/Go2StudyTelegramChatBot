...
3. **AI-нормализация** - Gemini AI определяет наиболее подходящую тему
4. **Создание новой темы** - только если все предыдущие этапы не дали результата

#### 📝 **Примеры обработки тем:**

| Тема из PDF | Результат | Метод |
|-------------|-----------|-------|
| "Порядок выполнения операций" | → "Порядок действий" | Синоним "порядок" |
| "Линейные уравнения (усложнённый уровень)" | → "Простейшие уравнения" | Синоним "уравнен" |
| "Операции с дробями и остатками" | → "Действия с дробями" | Синонимы "операци", "дроб" |
| "Процент" | → "Проценты" | Синоним "процент" |
| "Геометрия-Углы" | → "Углы" | Синоним "угол" |
...
- **AI-поддержка**: Умная обработка сложных и нестандартных случаев
- **Сохранение структуры**: Максимально использует предопределенную иерархию тем

Это обеспечивает единообразие в системе при работе с PDF файлами из разных источников.