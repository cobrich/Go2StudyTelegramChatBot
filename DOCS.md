...
### Исправление проблем с навигацией (Декабрь 2024)

**Исправленные проблемы:**
1. **Кнопка "В главное меню" не работала** - исправлен неправильный паттерн обработчика callback в `bot.py`
2. **Пользователи застревали на сообщении "Формируются вопросы"** - исправлена логическая ошибка с использованием переменной `topic` до её определения
3. **Отсутствие возможности выхода во время генерации вопросов** - добавлена кнопка "В главное меню" к сообщениям о формировании вопросов

**Технические детали:**
- Исправлен паттерн callback handler с `"^back_to_main$"` на `"^main_menu$"` в `src/bot.py`
- Перемещено определение переменной `topic` перед её использованием в `handle_topic_selection`
- Добавлены кнопки "В главное меню" ко всем сообщениям о генерации/формировании вопросов
- Улучшена обработка ошибок при выборе тем

### Автоматическая настройка пользователей из белого списка (Декабрь 2024)