...
- **Управление разнообразием через `task_type`:** Добавлен новый параметр `task_type` ("жизненная ситуация", "числовая прямая", "уравнение" и т.д.), который позволяет напрямую заказывать тип генерируемой задачи. Это дает полный контроль над разнообразием вопросов и решает проблему с отсутствием у AI "памяти" о прошлых генерациях.
- **Конкретизация требований:** В промпт добавлены более сильные негативные ограничения (запрет на избитые сценарии) и позитивные примеры (про квесты, космос, роботов), что направляет AI на создание более интересных и нетривиальных заданий.
