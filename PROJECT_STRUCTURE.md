...
```
web: python src/bot_universal.py
```
...
#### `debug.log` (113B, 2 строки)
Файл логирования для отладки

#### `question_images/` и `files/`
Директории для хранения изображений вопросов и файлов
...
#### `bot.py` (18KB, 521 строк)
**Главный файл запуска бота с полной функциональностью**

**Основные функции:**
- `main()` - точка входа в приложение
- `setup_handlers()` - настройка всех обработчиков
- `setup_admin_handlers()` - настройка админ-обработчиков
- `setup_error_handler()` - обработка ошибок
- `check_environment()` - проверка переменных окружения
...
#### `bot_universal.py` (10KB, 302 строки)
**Универсальная версия бота для облачного деплоя**
...
**Отличия от bot.py:**
- Упрощенная структура для облачных платформ
- Автоматическое определение режима (polling/webhook)
- Совместимость с Heroku, Railway

#### `bot_factory.py` (155B, 7 строк)
...
#### `bot_compat.py` (3.2KB, 92 строки)
**Слой совместимости для разных версий python-telegram-bot**

**Основные функции:**
- `create_universal_bot()` - создание бота с совместимостью
- `get_application_class()` - получение класса Application
- `setup_compatibility()` - настройка совместимости

#### `init_superadmin.py` (2.7KB, 64 строки)
...
### 🗄️ `database.py` (142KB, 3160 строк)
...
### 📄 `pdf_processor.py` (47KB, 922 строки)
...
### 🧠 `ai_service_improved.py` (26KB, 406 строк)
...
### 💬 `command_handlers.py` (44KB, 852 строки)
...
### 🌐 `translations.py` (9.3KB, 152 строки)
...
### 💬 `messages_kk.py` (8.3KB, 119 строк)
