...
class QuestionsHandler(AdminBaseHandler):
    ...
    async def pdf_format_guide(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        text += f"Тема: Название темы (количество вопросов)\n\n"
        ...
        text += f"• Каждая тема должна начинаться с 'Тема: название (количество)'\n"
        text += f"• Вопросы нумеруются: 1. 2. 3. и т.д.\n"
        text += f"• Варианты ответов: A) B) C) D) или А) Б) В) Г)\n"
        text += f"• Правильный ответ помечается галочкой ✅\n"
        text += f"• PDF должен содержать только текст (без изображений)\n"
        text += f"• Темы должны существовать в системе!\n\n"
        text += f"<b>Пример:</b>\n"
        text += f"Тема: Арифметические операции (2)\n\n"
        text += f"1. Сколько будет 2+2?\n"
        text += f"A) 3\n"
        text += f"B) 4 ✅\n"
        text += f"C) 5\n"
        text += f"D) 6\n\n"
        text += f"2. Сколько будет 3×3?\n"
        text += f"A) 6\n"
        text += f"B) 8\n"
        text += f"C) 9 ✅\n"
        text += f"D) 12"
