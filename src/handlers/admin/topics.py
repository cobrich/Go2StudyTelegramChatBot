...
class TopicsHandler(AdminBaseHandler):
    ...
    async def add_base_topics(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        # Получаем базовые темы из базы данных по языку
        base_structure = self.db.get_base_topic_structure_by_language(user_language)
        
        # Если база данных пуста, показываем сообщение
        if not base_structure:
            text = "❌ <b>Базовые темы</b>\n\n"
            text += "Базовая структура тем не найдена.\n"
            text += "Обратитесь к администратору для настройки базовых тем."
        ...
        if not missing_topics:
            ...
            text += f"🌐 Язык: {'Русский' if user_language == 'ru' else 'Казахский'}\n\n"
            ...
            text = "📋 <b>Базовые темы</b>\n\n"
            text += f"🌐 <b>Язык:</b> {'Русский' if user_language == 'ru' else 'Казахский'}\n"
            ...
            # Группируем по основным разделам и показываем до 10 тем
            current_main = None
            shown_topics = 0
            max_topics_to_show = 10
            
            for main_topic, subtopic in missing_topics:
                if shown_topics >= max_topics_to_show:
                    break
                ...
                # Создаем уникальный callback_data для каждой темы
                topic_hash = str(hash(subtopic))[-6:]  # Последние 6 символов хеша
                keyboard.append([InlineKeyboardButton(
                    f"➕ {subtopic}",
                    callback_data=f"add_base_topic_{topic_hash}"
                )])
                shown_topics += 1
            
            if len(missing_topics) > max_topics_to_show:
                text += f"\n... и еще {len(missing_topics) - max_topics_to_show} тем"
            
            # Сохраняем mapping для callback_data
            context.user_data['base_topics_mapping'] = {}
            for main_topic, subtopic in missing_topics:
                topic_hash = str(hash(subtopic))[-6:]
                context.user_data['base_topics_mapping'][topic_hash] = {
                    'name': subtopic,
    ...
    async def add_base_topic_execute(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        query = update.callback_query
        topic_hash = query.data.replace('add_base_topic_', '')
        await self.safe_answer_callback(query)
        
        # Получаем информацию о теме из mapping
        base_topics_mapping = context.user_data.get('base_topics_mapping', {})
        topic_info = base_topics_mapping.get(topic_hash)
        ...
        admin_id = update.effective_user.id
        user_language = self.db.get_user_language(admin_id)
        
        full_topic_name = topic_info['name']
        main_topic_name = topic_info['main_topic']
        
        # Добавляем тему с указанием языка и основного раздела
        description = f"Подтема раздела '{main_topic_name}'"
        success = self.db.add_topic(
    ...
    async def add_all_missing_topics_execute(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        for main_topic, subtopics in base_structure.items():
            for subtopic in subtopics:
                if subtopic not in existing_topics:
                    description = f"Подтема раздела '{main_topic}'"
                    success = self.db.add_topic(
                        name=subtopic,
                        description=description,
                        admin_id=admin_id,
        ...
        if added_count > 0:
            ...
            text += "\nВсе базовые темы теперь доступны в системе!"
        else:
