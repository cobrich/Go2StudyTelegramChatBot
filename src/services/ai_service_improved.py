...
4.  **Шығармашылық:** Әр сұрақта бірдей кейіпкерлерді (мысалы, Аня, Боря, Вера) немесе сценарийлерді (мысалы, достар туралы есептер) қайталамаңыз. Әртүрлі және қызықты тапсырмалар жасаңыз.
...
4.  **Креативность:** Не повторяй одних и тех же персонажей (например, Петя, Вася, Коля) или сценарии (например, задачи про друзей) в нескольких вопросах подряд. Создавай разнообразные и интересные задачи.
