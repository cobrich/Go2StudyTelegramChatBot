...
ДЕТАЛЬНЫЙ АНАЛИЗ СОДЕРЖАНИЯ ВОПРОСА:

🔢 АРИФМЕТИКА И ВЫЧИСЛЕНИЯ:
- Простые вычисления (сложение, вычитание, умножение, деление) → "Арифметические операции"
...
📊 ПРОЦЕНТЫ:
- Нахождение процента от числа ("Найдите 25% от 80") → "Нахождение процента от числа"
...
- Углы → "Углы"
- Единицы измерения (см, м, дм) → "Единицы длины и массы"

🚗 СПЕЦИАЛЬНЫЕ ТЕМЫ:
- Задачи на движение → "Движение"
- Масштаб карт → "Масштаб и расстояние"
- Пропорции и соотношения → "Пропорция и разность"
''' if sample_question else ''}

СТРОГИЕ ПРАВИЛА АНАЛИЗА:
1. ОБЯЗАТЕЛЬНО выбери тему ТОЛЬКО из списка выше
2. НЕ создавай новые темы
3. ПРИОРИТЕТ: анализируй СОДЕРЖАНИЕ вопроса, а не только название темы
4. Если вопрос содержит проценты (%, "процент от") → выбирай между "Проценты" и "Нахождение процента от числа"
5. Если вопрос с переменными и знаком равенства → "Простейшие уравнения"
6. Если вопрос на вычисление значения выражения → "Арифметические выражения"
7. Если простые арифметические вычисления → "Арифметические операции" или "Натуральные числа"
8. Если геометрические фигуры и измерения → "Геометрические фигуры" или "Периметр и площадь"

ПРИМЕРЫ ПРАВИЛЬНОГО СОПОСТАВЛЕНИЯ:
• "Найти значение 2x" + "Найдите значение 2x + 3, если x = 5" → "Арифметические выражения"
• "Порядок выполнения операций" + "Вычислите: 2 + 3 × 4" → "Порядок действий"
• "Линейные уравнения" + "Решите уравнение: 3x - 7 = 2x + 5" → "Простейшие уравнения"
• "Операции с дробями" + "Вычислите: 3/4 + 1/2" → "Действия с дробями"
...
• "Числовые выражения" + "Вычислите: 31.4 ÷ 7.9" → "Натуральные числа"
• "Арифметика-выражения с переменными" + "2x - y при x = 3" → "Арифметические выражения"

ВАЖНО: Если сомневаешься между несколькими темами, выбери более СПЕЦИФИЧНУЮ тему, которая точнее описывает содержание вопроса.
