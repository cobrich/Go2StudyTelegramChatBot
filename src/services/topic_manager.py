...
class TopicManager:
    ...
    def _normalize_topic_with_ai(self, topic_name: str, sample_question: str = None) -> str:
        ...
        try:
            ...
            prompt = f"""Ты эксперт по математике. Проанализируй математический вопрос и определи наиболее подходящую тему из списка.

ДОСТУПНЫЕ ТЕМЫ:
{topics_text}

ИСХОДНОЕ НАЗВАНИЕ ТЕМЫ: "{topic_name}"
ВОПРОС: "{sample_question or 'Не предоставлен'}"

ЗАДАЧА:
Исходное название темы может содержать несколько математических областей (например, "Арифметика - Дроби, проценты, уравнения").
Твоя задача - проанализировать конкретный вопрос и определить, к какой именно области он относится.

ПРИНЦИПЫ АНАЛИЗА:
1. Внимательно изучи содержание и структуру вопроса
2. Определи тип математических объектов в вопросе (числа, дроби, переменные, геометрические фигуры)
3. Определи тип операций (арифметические, алгебраические, геометрические)
4. Выбери наиболее точную тему из списка

КЛЮЧЕВЫЕ ПРИЗНАКИ:
• Десятичные дроби: числа с запятой (2,46; 0,253; 37,46) и операции с ними
• Обыкновенные дроби: дроби вида a/b (1/2, 3/4, 5/8) и операции с ними  
• Проценты: символ % или слова "процент", "процентов" в самом вопросе
• Уравнения: переменные (x, y) и знак равенства для их нахождения
• Арифметические операции: вычисление значений выражений с переменными при заданных значениях
• Порядок действий: сложные арифметические выражения со скобками и приоритетами операций
• Геометрия: фигуры, периметр, площадь, углы, измерения
• Натуральные числа: работа с целыми положительными числами

ПРИМЕР АНАЛИЗА:
Исходная тема: "Арифметика - Дроби, проценты, уравнения"
Вопрос: "2,46 × 18 ="
Анализ: В вопросе есть десятичное число 2,46 и операция умножения
Результат: "Десятичные дроби"

Проанализируй вопрос и верни ТОЛЬКО название темы из списка выше."""
            ...
            if normalized_topic in all_topics:
                logger.info(f"AI нормализация: '{topic_name}' → '{normalized_topic}'")
