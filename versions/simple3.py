...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "👋 Привет! Я бот для изучения математики. Выбери действие:",
...
async def go_to_main_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- BEGIN EDIT: Изменяем текст сообщения для главного меню ---
    ...
    try:
        ...
        await context.bot.send_message(
            ...
            text=menu_message_text, # Используем измененный текст
            reply_markup=main_menu_markup
        )
        logging.info(f"Сообщение '{menu_message_text}' с главным меню (ReplyKeyboardMarkup) отправлено в чат {query.message.chat.id}")
        
        # После успешной отправки нового сообщения с меню,
        # удаляем старое сообщение, на котором была инлайн-кнопка "В главное меню".
        try:
            await query.delete_message()
            logging.info(f"Предыдущее сообщение (ID: {query.message.message_id}) удалено в чате {query.message.chat.id}")
        except telegram.error.BadRequest as e_del_br:
            if "message to delete not found" in str(e_del_br).lower():
                logging.info(f"Сообщение (ID: {query.message.message_id}) для удаления в go_to_main_menu_callback уже было удалено.")
            elif "message can't be deleted" in str(e_del_br).lower() or "message is not modified" in str(e_del_br).lower():
                logging.warning(f"Сообщение (ID: {query.message.message_id}) не может быть удалено или не найдено: {e_del_br}")
            else:
                logging.warning(f"Не удалось удалить предыдущее сообщение (BadRequest) в go_to_main_menu_callback: {e_del_br}")
        except Exception as e_del:
            logging.warning(f"Не удалось удалить предыдущее сообщение (Другая ошибка) в go_to_main_menu_callback: {e_del}")

    except Exception as e_send:
        logging.error(f"Ошибка при отправке сообщения с главным меню в go_to_main_menu_callback: {e_send}")
        # Запасной вариант, если отправка с клавиатурой не удалась
        try:
            await context.bot.send_message(
                chat_id=query.message.chat.id,
                text="Произошла ошибка при отображении главного меню. Пожалуйста, попробуйте команду /start."
            )
